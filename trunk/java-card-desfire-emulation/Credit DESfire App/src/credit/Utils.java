...
public class Utils {
	...
	//Transmission modes
	 ...
	 final static short BUFFER_DATA_LENGTH_MAX=128;
...
//	}
	...
	public static String hexDumpSpaces(byte[] data, int offset, int length) {
		String result = "";
		String part = "";
		for (int i = 0; i < be.fedict.util.MathUtils.min(data.length, length); i++) {
			if(i!=0) result=result+" ";
			part = hexChars[(byte) (be.fedict.util.MathUtils.unsignedInt(data[offset + i]) / 16)] + hexChars[(byte) (be.fedict.util.MathUtils.unsignedInt(data[offset + i]) % 16)];
			result = result + part;
		}
		return result;
	}
	...
	public static String hexDump(byte data) {
		String result = "";
		result = "" + hexChars[(byte) (be.fedict.util.MathUtils.unsignedInt(data) / 16)] + hexChars[(byte) (be.fedict.util.MathUtils.unsignedInt(data) % 16)]+"";
		return result;
	}
	/**
	...
	public static byte[] crc16(byte[] a){
	    ...
	    for (short i = 0; i < a.length; i++) {
            crc = (short) ((crc >>> 8) ^ table[(crc ^ a[i]) & (short) 0xff]);
        }
